...
resource "aws_internet_gateway" "gw" {
  vpc_id = aws_vpc.main.id
  tags   = merge({ "Name" = "${var.project}-ig" }, var.tags)
}

# VPC RouteTable
resource "aws_route_table" "alb" {
  vpc_id = aws_vpc.main.id
  tags   = merge({ "Name" : "${var.project}-route-alb" }, var.tags)
}
...
resource "aws_route_table" "private" {
  vpc_id = aws_vpc.main.id
  tags   = merge({ "Name" : "${var.project}-route-private" }, var.tags)
}
